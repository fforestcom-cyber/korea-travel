...
const LIST_COL = collection(db, 'checklists');
...
const ChecklistPage = () => {
  ...
  useEffect(() => {
    const q = query(LIST_COL, orderBy('createdAt', 'desc'));
    const unsub = onSnapshot(
      ...
      (err) => {
        console.error('checklists onSnapshot error:', err);
        setLoading(false);
      }
    );
    return () => unsub();
  ...
  const addItem = async () => {
    ...
    try {
      ...
      if (pendingFile) {
        setUploading(true);
        const result = await uploadImage(pendingFile, 'korea-travel/checklists');
        imageUrl = result.url;
        setUploading(false);
      }
      const dotColor = DOT_COLORS[colorIdx.current % DOT_COLORS.length];
      colorIdx.current += 1;
      await addDoc(LIST_COL, { text, imageUrl, dotColor, createdAt: serverTimestamp() });
      setDraft('');
      setPendingImg('');
      setPendingFile(null);
  ...
  const saveEdit = async (item: ListItem) => {
    ...
    setEditSaving(true);
    try {
      let imageUrl = editKeepImg ? item.imageUrl : '';
      if (editPendingFile) {
        setEditUploading(true);
        const result = await uploadImage(editPendingFile, 'korea-travel/checklists');
        imageUrl = result.url;
        setEditUploading(false);
      }
      await updateDoc(doc(db, 'checklists', item.id), { text, imageUrl });
      setEditingId(null);
      setEditDraft('');
      setEditPendingImg('');
      setEditPendingFile(null);
    ...
    <>
      ...
      <div className="section-px">
        ...
        <div className="note-compose mb-6">
          <textarea
            placeholder="有什麼想記下來的嗎？購物清單、待辦事項…"
            value={draft}
          ...
          <div key={item.id} className="note-card">
            <div className="note-card__body">
              <p className="note-card__date">
                <span className={`note-dot note-dot--${item.dotColor}`} />
              ...
              {editingId === item.id ? (
                <>
                  ...
                  {item.imageUrl && editKeepImg && !editPendingImg && (
                    <div style={{ position: 'relative', marginBottom: 10 }}>
                      <img
                        src={item.imageUrl}
                        alt="目前圖片"
                ...
                <>
                  ...
                  {item.imageUrl && (
                    <img src={item.imageUrl} alt="清單附圖" className="note-card__img" />
                  )}
                </>
              )}
            </div>
