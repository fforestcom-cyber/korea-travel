...
function extractBlocks(section) {
  ...
  for (const child of section.children) {
    ...
    if (cl.contains('info-card')) {
      const variant = ['accent', 'teal', 'gold', 'blue', 'amber', 'red'].find(v => cl.contains(v)) ?? 'blue';
      blocks.push({
        kind: 'info-card',
        variant,
      ...
      blocks.push({
        ...
        links: Array.from(child.querySelectorAll('.photo-link')).map(a => ({
          text:  txt(a),
          href:  a.getAttribute('href') ?? '',
          style: ['teal', 'amber', 'red', 'blue'].find(c => a.classList.contains(c)) ?? 'default',
        })),
      });

    } else if (cl.contains('facility-grid')) {
      blocks.push({
        kind: 'facility-grid',
        items: Array.from(child.querySelectorAll('.facility-card')).map(card => {
          ...
          const name = nameEl
            ? (nameEl.innerHTML ?? '')
                ...
                .trim()
            : '';
          const tags = Array.from(card.querySelectorAll('.facility-tag')).map(tag => ({
            label: txt(tag),
            type:  tag.classList.contains('f-thrill') ? 'thrill'
                 : tag.classList.contains('f-relax')  ? 'relax'
                 : 'must',
          }));
          return { name, tags, note: txt(card.querySelector('.facility-note')) };
        }),
      });

    } else if (cl.contains('highlight')) {
      blocks.push({
        kind:  'highlight',
        title: txt(child.querySelector('.highlight-title')),
        body:  txt(child.querySelector('.highlight-body')),
      });
    }
  }
...
function parseDayPlan(htmlPath, dayNumber) {
  ...
  const sections = allSections
    ...
    .map(s => ({
      ...
      mapQuery:  s.getAttribute('data-map-query') ?? '',
...
// ── run ────────────────────────────────────────────────────────────────
// Support: node scripts/import-schedule.mjs [1|2|3|4|5]
// No argument → process all 5 days

const arg = process.argv[2];
const days = arg ? [parseInt(arg, 10)] : [1, 2, 3, 4, 5];

console.log('');
for (const n of days) {
  const htmlPath = join(ROOT, `src/data/import/day${n}_plan.html`);
  const outPath  = join(ROOT, `src/data/scheduleDay${n}.ts`);
  const varName  = `day${n}Plan`;

  console.log(`Parsing: day${n}_plan.html`);
  const data = parseDayPlan(htmlPath, n);

  const output = `// AUTO-GENERATED by scripts/import-schedule.mjs — do not edit manually
import type { DayPlan } from '../types/dayPlan';

const ${varName}: DayPlan = ${JSON.stringify(data, null, 2)};

export default ${varName};
`;

  writeFileSync(outPath, output, 'utf-8');
  console.log(`  → scheduleDay${n}.ts  (timeline: ${data.timeline.length}, sections: ${data.sections.length})`);
}
