...
> **Build 注意**：Windows 路徑大小寫不一致（`D:\Codex` vs `D:\Codex`）會造成 ESLint plugin 衝突，建置時必須加 `DISABLE_ESLINT_PLUGIN=true`。
...
專案內有 `.Codex/` 目錄，存放 Codex 的對話設定與記憶，請勿手動刪除。
